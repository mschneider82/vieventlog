...
async function loadTemperatureData(silent = false) {
    ...
    try {
        ...
        // Fetch data from API
        const response = await fetch(`/api/temperature-log/data?installationId=${currentInstallationId}&hours=${hours}&limit=2000`);
        if (!response.ok) {
...
function updateAvailableFields(data) {
    ...
    if (selectedFields.size === 0) {
        const defaultFields = ['outside_temp', 'dhw_temp', 'primary_supply_temp', 'return_temp',
                              'compressor_active', 'thermal_power', 'cop'];
