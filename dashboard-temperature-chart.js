...
//RS		
        let hasHotWaterBuffer = false; // default: show heating circuit (ODU)
        // Get device settings
		...
		if (hasHotWaterBuffer){ defaultFields = ['outside_temp', 'secondary_supply_temp', 'return_temp', 'dhw_temp', 'buffer_temp'];}
...
function renderFilters() {
    ...
    const fieldLabels = {
        ...
        'calculated_outside_temp': '🌡️ Außentemp. (ged.)',
    ...
    const categories = {
        'Temperaturen': ['outside_temp', 'calculated_outside_temp', 'secondary_return_temp', 'primary_supply_temp', 'secondary_supply_temp',
                        'return_temp', 'primary_return_temp', 'dhw_temp', 'dhw_cylinder_middle_temp', 'boiler_temp', 
...
function renderTemperatureChart(data) {
    ...
    const fieldConfig = {
        ...
        'primary_supply_temp': { type: 'line', yAxisIndex: 0, color: '#ea4335', smooth: true },
        'secondary_supply_temp': { type: 'line', yAxisIndex: 0, color: '#ff6f00', smooth: true },
        ...
        yAxis: [
            {
                ...
                name: 'Temperatur (°C)',
       			nameTextStyle: {color: "#ffffff"},
                position: 'left',
				min: function(value){ return Math.floor(0.9*value.min);},
				max: function(value){ return Math.ceil(1.1*value.max);},
                axisLabel: {
   				    color: "#ffffff",
                    formatter: '{value} °C'
                }
