...
// fetchFeaturesForDeviceWithTracking wraps fetchFeaturesForDevice with API call tracking
func fetchFeaturesForDeviceWithTracking(installationID, gatewayID, deviceID, accessToken string) (*DeviceFeatures, error) {
	trackAPICall()
	return fetchFeaturesForDevice(installationID, gatewayID, deviceID, accessToken)
}
...
func extractFeatureIntoSnapshot(feature Feature, snapshot *TemperatureSnapshot) {
	...
	// Compressor data
	case "heating.compressors.0.active":
		snapshot.CompressorActive = getBoolValue(feature.Properties)
	case "heating.compressors.0.statistics.speed":
	...
	case "heating.compressors.0.sensors.pressure":
	...
	case "heating.compressors.0.sensors.temperature.motor":
	...
	case "heating.compressors.0.statistics.hours":
		snapshot.CompressorHours = getFloatValue(feature.Properties)
...
// calculateDerivedValues computes COP and thermal power
func calculateDerivedValues(snapshot *TemperatureSnapshot) {
	...
	if snapshot.VolumetricFlow != nil && snapshot.SupplyTemp != nil && snapshot.ReturnTemp != nil {
		// Thermal Power (kW) = Flow (L/h) * ΔT (K) * 4.186 (kJ/kg·K) / 3600 (s/h)
		// Simplified: kW = Flow * ΔT * 0.001163
		deltaT := *snapshot.SupplyTemp - *snapshot.ReturnTemp
		thermalPower := *snapshot.VolumetricFlow * deltaT * 0.001163
		snapshot.ThermalPower = &thermalPower

		// Calculate COP if we have compressor power
		if snapshot.CompressorPower != nil && *snapshot.CompressorPower > 0 {
			cop := thermalPower / *snapshot.CompressorPower
			snapshot.COP = &cop
		}
	}
}
